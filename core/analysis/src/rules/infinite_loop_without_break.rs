    ...
    fn check(ctx: &TypedContext) -> Vec<LabeledDiagnostic> {
        ...
        for source_file in ctx.source_files() {
            ...
            crate::walker::for_each_function_body(arena, &source_file.defs, &mut |body_id| {
                check_block(arena, module_path, body_id, &mut errors);
            });
        }
        errors
    }
...
fn check_block(
    ...
    check_statements(arena, module_path, &block.stmts, errors);
}
...
fn check_statements(
    ...
    for &stmt_id in stmt_ids {
        check_statement(arena, module_path, stmt_id, errors);
    }
}
...
fn check_statement(
    ...
    match &arena[stmt_id].kind {
        Stmt::Loop { condition, body } => {
            ...
            check_block(arena, module_path, *body, errors);
        }
        Stmt::If {
            then_block,
            else_block,
            ...
            check_block(arena, module_path, *then_block, errors);
            if let Some(else_id) = else_block {
                check_block(arena, module_path, *else_id, errors);
            }
        }
        Stmt::Block(block_id) => {
            check_block(arena, module_path, *block_id, errors);
        }
        Stmt::Assign { .. }
        | Stmt::Return { .. }
        | Stmt::Break
