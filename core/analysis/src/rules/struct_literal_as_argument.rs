//! A013: Struct literals cannot be passed directly as function arguments.
//!
//! The codegen requires a named variable for frame slot allocation, so struct
...
crate::rule! {
    /// Struct literals cannot be passed directly as function arguments.
    #[id = "A013"]
    #[name = "Struct literal as argument"]
    #[severity = error]
    pub struct StructLiteralAsArgument;
    ...
    match &arena[expr_id].kind {
        Expr::FunctionCall { function, args, .. } => {
            ...
            for (_, arg_expr) in args {
                if matches!(arena[*arg_expr].kind, Expr::StructLiteral { .. }) {
                    errors.push(AnalysisDiagnostic::StructLiteralAsArgument {
