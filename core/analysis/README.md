...
Type aliases are a documented non-scope in both forms, the item `type A = Q i32';` and the body-level `type X = Q i32';`, for A045's and A048's reason plus a second: a body-level alias is erased in code generation. Function types (`fn(i32) -> i32`) are refused for their own reason, with or without a type parameter in them, and this rule does not claim them.
