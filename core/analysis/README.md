...
The type name is kept and the values are rejected, so an author who writes `string` is told the feature is not implemented and what to model text with instead — a `[u8; N]` with its bytes written as numbers, or an enum tag when the value is one of a fixed set — rather than being told `string` is an unknown type. Type positions are read from the annotation as written rather than from the resolved struct table, because the predicate is a builtin type kind that `TypeInfo::from_type_id` decides on its own; the binding half reads the type the checker recorded, which is the resolved one. One documented non-scope: the `self` receiver, whose type is the enclosing struct. `spec` bodies are covered — a spec function is lowered to a real WebAssembly function in proof mode and reaches the same expression lowering — and the proof translation's own rejection covers a string literal in an *assertion term*, which is a different path. The rule is a gate on an unimplemented feature: the day strings are implemented, it is deleted whole.

### Unit Values (errors)

| ID | Struct | Severity | What it checks |
|----|--------|----------|----------------|
| A049 | `UnitAsValue` | error | a `()` value: a unit literal outside the two exempt statement forms, or `()` as the type of a binding, parameter, or struct field |

A049 draws the line between the *absence* of a value and a *value of nothing*. The first is the point of the unit type and is implemented: a function whose return type is `()` or omitted returns nothing, and code generation gives it an empty result list. The second is a declaration that cannot be honoured — a unit value carries no information, so it occupies no bytes and has no WebAssembly type. A parameter declared `()` is given no argument slot to arrive in, a binding of it has nothing to store, an array of it has no element size for frame layout to compute, and a struct field of it has no offset that means anything.

Covered: a unit literal, the recorded type of a `let` or of a `const` at function or module scope, a function/method/`external fn` parameter (including `_: ()`), and a struct field — arrays peeled at any depth. The type has one spelling, `()`: `unit` is a reserved word the parser refuses before analysis runs. **The return type is not covered**, because it is the one place unit means something. Two statement forms are exempt: a unit literal as the whole expression of a `return` or of an expression statement, parentheses peeled. That exemption is load-bearing rather than cosmetic — the parser synthesizes a unit literal for the missing expression of a bare `return;`, so a rule that rejected the literal unconditionally would reject every void function ever written. The exemption reaches the root and nothing below it, so `f(())` is still reported.
...
| ID | Struct | Severity | What it checks |
|----|--------|----------|----------------|
| A051 | `GenericNotSupported` | error | a `fn` declaring type parameters (`fn id T'`), or a type application (`Q i32'`) written as a type or standing in expression position |

A051 refuses generic code before code generation. The type checker handles generics in full — at each call site it infers a type argument from the arguments it was given and checks the body against it — and nothing carries that substitution any further. The compiler does not monomorphize, so a type parameter arrives at code generation still standing for no type at all: no layout to size a frame slot with, no WebAssembly value type to pass a value in, no term for a proof to describe one with. A type *argument* is worse off still, because no type declaration in the language accepts one: a struct, an enum and an `external fn` each declare a bare name, and only a function binds a type parameter, so `Q i32'` names no declaration at all.

What that produced before the rule was three outcomes, and only the first was a failure. A binder appearing in a lowered type reached an unsupported-type refusal with no location, because signature lowering sees a type node and no enclosing type parameters and so cannot tell `T` from a misspelled type name. A binder appearing nowhere lowered — `fn h T'(x: i32)`, `pub fn main T'()` — was emitted, exported, and in proof mode shipped a complete obligation with the binder discarded, while being uncallable from Inference: with the binder absent from the parameter list nothing can be inferred, so the type checker refuses every call. And a binder shadowing a declared type was read one way by the type checker and the other by code generation, producing a program that exits 0 and emits a module WebAssembly validation rejects.

Covered: a `fn` declaring type parameters — free function, struct method, `spec` function, or method of a struct declared inside a `spec` — as one finding per declaration caretted on the first binder; a type application as a function/method/`external fn` parameter or return type, as a struct field, or as the declared type of a `let` or of a `const` at function or module scope; and a type application in expression position at any depth, which is why the expression half is a recursive descent rather than a pass over statement roots — `(Q i32');` puts the type inside a parenthesization. Arrays are looked through at any depth, one finding on the annotation that carries the element. The declaration predicate is the declared binders and nothing else: no reachability filter and no entry-point carve-out, since each candidate narrowing leaves one of the three outcomes above standing. Reporting anchors on declarations and annotations, never on call sites, on the principle A045 states; where a declaration is both (`fn g T'(p: Q T')`) the two findings are separate, because the binder and the application are two different edits. `spec` bodies are covered for A048's reason. The rendered spelling comes from the arena rather than from the type the checker recorded, whose rendering puts a tick on the base as well (`Q' i32'`) — a spelling the grammar rejects — and drops the arguments when it peels array nesting.

Function types (`fn(i32) -> i32`) are refused for their own reason, with or without a type parameter in them, and this rule does not claim them.
...
6. Add the rule to the catalog in `src/lib.rs`'s module documentation, under the family it belongs to.
7. Add a row to the rule table of that family above, and a row to the test-file table below.
8. Add `tests/src/analysis/rules_aNNN.rs` in `inference-tests` and declare it in `tests/src/analysis/mod.rs`.
9. Add the rule to `book/src/static-analysis.md` — a row in its family table, a paragraph stating the argument for it, and the counts at the head of *Current Rules*, which are re-derived from `all_rules()` rather than incremented — and extend the `A001`…`ANNN` range in `book/src/the-language-server.md`.
10. Add a `CHANGELOG.md` entry under *Analysis*, ending with the new `all_rules()` count.
11. If a code generation backstop names the id in a `rule:` field, add the id to `NAMED_ANALYSIS_RULES` in `core/wasm-codegen/src/errors.rs`; a test pins that list against the sources in both directions.
