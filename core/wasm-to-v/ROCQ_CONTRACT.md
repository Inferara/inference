...
## Verification claims and the selected-artifact protocol

Three different checks apply to emitted Rocq, and their claims must not be
collapsed:

1. **Vendored-stub elaboration** compiles generated `.v` against the
   deliberately narrowed declarations in `rocq-stub/`. It checks emitted
   syntax and the interface subset, including that the emitter does not grow
   into real-library declarations which the stub intentionally omits. It does
   not prove an obligation.
2. **Pinned real-library elaboration** compiles the same generated bytes
   against the pinned wasm-verifier and coq-wasm libraries. It checks the real
   consumer interface and load path. It still does not prove an obligation:
...
3. **Exact-artifact discharge** is the stronger, narrowly scoped claim for the
   ...
   and audits every rebound endpoint for raw-namespace and unapproved
...
| Case ID | Raw artifact | Generated theorem -> companion endpoint | Result |
| --- | --- | --- | ---: |
...
aggregate floors, and for each ordered case its ID, basename, fresh raw
SHA-256, and proved/refuted counts. Publication is transactional, and the
request plus raw bytes are fingerprinted across every external bridge call.

The configured single-case executable is invoked exactly as

```text
$INFERENCE_WASM_VERIFIER_DISCHARGER \
  --protocol 1 \
  --wasm-verifier-revision <canonical-40-hex> \
  --case <stable-case-id> \
  <fresh-raw-file.v>
```

and writes only `<case-id>.json` in the new empty absolute directory named by
`INFERENCE_WASM_VERIFIER_RECEIPT_DIR`. Exit zero is insufficient: the Task 4
shell wrapper owns the directory/file identity evidence and requires each
...
fail. The local Docker inspector is separately exact at `8.20.1`. Before any
normal result is trusted, a deliberately malformed same-basename provenance
probe must fail.
...
and asserts Rust 1.98. It copies the checkout into a disposable source volume,
installs the tracked `ci/rocq-discharge.cargo-lock` as that snapshot's root
lock, verifies its hash, fetches with `--locked`, and then runs every build and
test offline and locked with networking disabled. The ignored checkout-root
...
and final verification does not alter this Docker wrapper contract; every Rocq,
proof, and bridge/discharge command remains Docker-only.

The immutable cross-repository sequence is A -> B -> C. A defines the
producer, fixtures, strict request/receipt validator, and Docker wrapper but
