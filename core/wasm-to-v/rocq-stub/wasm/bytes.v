...
(* Data-segment contents reach the `.v` as byte literals, one Rocq name per
   source byte (`x78 :: nil`). The names mirror the standard library's
   `Byte.byte` constructors -- `x00` .. `xff`, lowercase hex -- so an emitted
   `moddata_init` list elaborates unchanged once the real WasmCert library,
   whose `byte` *is* the standard library's, replaces this stub. Here they stay
   opaque parameters of the opaque `byte` above: the stub type-checks the
   emitted shape without modelling byte values. *)
Parameter x00 x01 x02 x03 x04 x05 x06 x07 x08 x09 x0a x0b x0c x0d x0e x0f
          ...
          xf0 xf1 xf2 xf3 xf4 xf5 xf6 xf7 xf8 xf9 xfa xfb xfc xfd xfe xff : byte.
