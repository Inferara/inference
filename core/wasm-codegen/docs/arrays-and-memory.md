...
The guard is emitted for **every build `codegen()` drives, at every target**: Compile and Proof mode, Debug and Release alike. The `codegen()` entry point's emission path sets `Compiler::emit_bounds_checks` unconditionally, and nothing on that path reads the target at all, so the deployed artifact is always checked and the artifact a proof is written about is byte-for-byte the artifact that ships. `OptLevel` does not influence bounds checks.
