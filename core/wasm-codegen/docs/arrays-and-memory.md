...
The guard is emitted for **every build**: Compile and Proof mode, Debug and Release, Wasm32 and Stellar. The `codegen()` entry point's emission path sets `Compiler::emit_bounds_checks` unconditionally, so the deployed artifact is always checked and the artifact a proof is written about is byte-for-byte the artifact that ships. `OptLevel` does not influence bounds checks.
