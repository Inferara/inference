...
the emitted binary. Every target but `Wasm32` rejects the opt-in, each for a reason of its own: `Stellar`
refuses it deliberately, not for want of evidence about its runtime, which does accept the
opcodes, while `SpaceWasm`'s decoder has not implemented the proposal at all. What they
share is what makes the answer the same: keeping a target's output inside the WebAssembly
1.0 instruction set is what every deployment path accepts, and no lowering any target can
reach needs the instructions.
...
- `lib.rs` - Public API, multi-file AST traversal in canonical arena order, two-stage index pre-scan across all files (imports → top-level functions → methods), root-only export policy (`should_export`), file-qualified spec name emission (`qualified_spec_name` with `_` join for non-entry specs), `SpecNameCollision` backstop, proof-mode `name`-section symbol-collision backstop (`check_name_section_symbols`) — refuses two non-spec functions that render one `FnKey::name_section_symbol()` only when a `T_app`/`HA_app_ok` obligation actually applies that shared symbol, so a coincidental collision no obligation names is left alone; the Stellar admissibility gate over the export descriptor (`check_stellar_exports`), which refuses, among its rules, two parameters that claim one `stellar` CLI flag
...
- `output.rs` - `CodegenOutput` containing WASM bytes, metadata, and (proof mode only) the per-spec `hassert` obligation map (`hspecs()`); also carries the source-level signature of every exported function (`export_signatures()`, `ExportSignature`, `AbiParam`, `AbiType`, `AbiReturn`) — a pure observation absent from the emitted bytes, read by the Stellar admissibility gate below and by `inference-stellar-abi`'s Val-ABI rewrite
- `target.rs` - Compilation target definitions (`Wasm32`, `Stellar`, `SpaceWasm`) and the requestable post-MVP instruction families (`EmitFeatures`)
- `stellar_cli.rs` - What `stellar contract invoke` derives from a parameter list: `stellar_cli_flag_alias`, the kebab-case second flag it gives a parameter — a transcription of `heck` 0.5.0's `to_kebab_case` — and `stellar_flag_collision`, the first two parameter names that claim one flag, which the Stellar gate here and `inference-stellar-abi`'s rewriter both refuse; the refusal itself stays in each gate
