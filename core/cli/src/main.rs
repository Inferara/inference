...
/// artifact written from those bytes would describe a module whose exports
/// point at different functions, with none of the value-ABI wrappers and none
/// of the three contract sections the rewrite appends, while claiming to be
/// about the artifact beside it on disk. That is worse than no `.v` at all, so
/// the pairing is refused rather than qualified.
///
...
fn proof_artifact_refusal(
    ...
    Some(format!(
        "Error: -v cannot be combined with --target {}. The module this target \
         ...
         module whose exports point at different functions, with none of the \
         value-ABI wrappers and none of the three contract sections the \
         rewrite appends, not the .wasm written beside it. Build the same \
         source at --target {} to obtain a .v — code generation is \
         target-blind, so the pre-rewrite bytes of the two builds are the same \
         module.",
