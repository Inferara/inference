...
| `[wasm-dependencies]` | Each entry is forwarded as `--wasm-dep <name>=<path>`, with the declared path resolved against the project root; honored on every build and run path, and never capability-gated. A key whose first segment is `host` is refused on every build and run path before `infc` is asked to build: that segment names the embedder in a `use … from` clause |
| `[host-imports]` | The host-import allowlist, one array of field names per import module (`env = ["clock_ms"]`). Forwarded as one argument, `--host-imports=<module.field,…>` sorted by module and field, on every build and run path, and echoed as a `host-imports allowlist:` line; a table with no keys forwards `--host-imports=` and admits no host function. Absent, no policy applies. Validated when the manifest is loaded, not where it is forwarded, and requires `infc` ABI 1.8 or newer — an older compiler is refused rather than built without the policy |
| `[build.wasm-opt]` | Opt-in post-build optimization of `out/main.wasm` via Binaryen `wasm-opt`, resolved from `WASM_OPT_PATH` → PATH → an infs-managed install; absent table is a no-op. An optimized artifact's overflow-guard record is rewritten to say it can no longer name its guarded functions, so linking it as a library refuses reachability specifications rather than checking a stale list. Since `+`, `-`, `*` and unary `-` trap unless written `wrapping(...)`, that reaches any library with arithmetic in it. When the step runs at any level but `"0"`, the optimized artifact imports only the host functions the program calls, and the build log names each host import the optimizer removed and the set that ships |

Five manifest settings are also honored in **single-file** mode, by walking up to the nearest `Inference.toml`: `[wasm-dependencies]`, `[build] target`, `[build] wasm-features`, `[memory]` and `[host-imports]`. `[build] wasm-features` is honored by both `infs build <path>` and `infs run <path>` — deliberately, since those two and project `infs build` all write `out/main.wasm` for the same project and must not disagree about its instruction set — and `[build] target` and `[memory]` are honored by both for the same reason. `[wasm-dependencies]` is honored on all four compilation paths — single-file `build`, single-file `run`, project `build`, and project `run` — and is never capability-gated; `[host-imports]` is honored on the same four, so running one file of a project never skips the policy building it applies. `[build] wasm-features` requires an `infc` with ABI 1.2 or newer; an older compiler cannot honor the request, so it is refused with remediation instead of being handed the flag.
...
Where the build runs is its `[build] target`'s to decide. A `wasm32` build runs under `wasmtime`, which must be installed. A `spacewasm` build runs in process under the SpaceWasm flight interpreter, embedded through `inference-spacewasm-runner` at the reference embedder's configuration, and needs no runtime of its own. A `stellar` build is refused before anything is built. On either runtime the invoked function's return value is the last line on stdout, as a signed decimal: a `u32` or `u64` result above its type's signed maximum prints as the negative number with the same bits, so a `u32` of 4294967295 prints `-1`. The exit status is not that value: it is 0 whenever the call returns.

On a `spacewasm` build each F´ (F Prime) reference host but `clock_ms` logs its calls to stderr as they are made, and a refused import, a trap or an exhausted `--fuel` budget exits with status 1. The book's [Running a SpaceWasm build](../../book/src/compilation_targets.md#running-a-spacewasm-build) walks through a first run — a program calling the reference hosts, built and run with its real output — and sets out what each host does, what a run shows and what it does not.
