...
- [ ] Preserve `pull_request` rather than `pull_request_target`, keep no path filter, and document that the label only schedules the job while environment approval explicitly trusts the pull request code; neither the environment nor runner group isolates that code, so the selected runner must be one-job ephemeral and carry no unrelated state or credentials.
...
- [ ] Compile and test Rust on the local host with exact Rust 1.98 and the tracked lane lock, as explicitly requested for this implementation. Invoke pinned verifier B's adapter from that host-built direct gate so Rocq/Coq proof work remains Docker-only; also exercise B's Docker batch direction against immutable commit A. Do not use the canonical product wrapper's Docker Rust build as evidence for this local run.
