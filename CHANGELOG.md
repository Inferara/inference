...
- Runtime array bounds checking for dynamic indices — the dynamic half of array bounds checking ([#164])
  ...
  - Gated on `OptLevel::O0` (the build-profile matrix maps `O0 ⟺ Debug ⟺ checks-on`): the `Compiler` derives an `emit_bounds_checks` flag from the `opt_level` already passed to `codegen()` — no new parameter and no dependency on the CLI-layer `BuildProfile`. **Release** (`O3`/`Oz`) and **Proof** (always a release opt level) output is byte-identical to an unchecked build, so the verified artifact stays the deployed artifact
  - The scratch i32 local is reserved immediately after the frame-pointer temp, only when bounds checks are on, so Release local declarations (and emitted bytes) are unchanged. The `unreachable` trap reuses the `assert` idiom and maps to `BI_unreachable` in the Rocq translator, keeping guarded code translatable. New `wasm_codegen_emit_bounds_check` cov-mark. Constant indices are not guarded here — they are rejected statically by analysis rule A037
  - Treating dynamic bounds as discharged Rocq proof obligations (rather than runtime traps) is reserved future work; the `emit_index_offset` choke point is the seam where a Proof-mode path would hook in
...
- A037 `ArrayIndexConstOutOfBounds`: reject a constant array index (`arr[c]`) that is negative or `>= length`, the static half of array bounds checking ([#164])
  ...
  - Dynamic (non-literal) indices are out of scope for the static rule and are guarded at run time in Debug builds (see Codegen)
