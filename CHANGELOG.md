...
- New analysis rule **A044** (`ShiftCountOutOfRange`, error) rejects a shift whose count is a literal (including parenthesized and negated literals) that is negative or greater than or equal to the operand type's bit width, e.g. `x << 32` or `x >> -1` on `i32` — such a shift never means what it says. Programs that previously compiled with such counts are now rejected. Literal counts currently type-check only for `i32`-typed shifts (binary operands do not coerce), but the rule covers all widths. Const-declared counts (`const K: i32 = 33; x << K`) are not detected — the same statically-known-literal scope as the division-by-zero check and A022.
...
- Proof mode (`-v`) now rejects the `unique` non-deterministic block with a clean diagnostic instead of silently emitting an un-verifiable `.v`. The wasm-verifier library defines no `BI_unique` constructor (it is commented out in its `datatypes.v`), so the previous proof-mode "success" produced a `BI_unique` reference that type-checked only against the repo's vendored Rocq stub and could never be consumed by the real verifier. A proof-mode build of a spec containing `unique` now exits 1 with `unsupported WASM feature: the `unique` non-deterministic block (…)` and writes neither `.wasm` nor `.v` (translation runs before any artifact write). `forall`/`exists`/`assume` blocks and uzumaki draws still translate; compile-mode builds are unchanged (specs are stripped from codegen, byte-identical output). Linking is unchanged: an external's executable body carrying `unique` was already rejected by the linker's allow-list, and a main module's proof scaffolding still links verbatim — the rejection happens at translation. The vendored stub's `BI_unique` arity pin is removed with this change, so any regression that re-emits `BI_unique` fails the `coqc` corpus gate as an unbound constructor.
