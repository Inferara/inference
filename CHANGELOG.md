...
- `ensure_infs_metadata_does_not_overwrite_existing` is now `#[serial_test::serial]`. Its deliberately drifted `schema_version` drives `read_infs_metadata` into the branch that reads `INFS_VERBOSE`, and `serial_test` serializes only *marked* tests — so it could read that variable while a marked test mutated it, which is undefined behavior rather than merely a flaky assertion ([#331])
