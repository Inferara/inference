...
- The emitted `.v` now spells every index immediate with an explicit `%N` scope (`BI_br`, `BI_br_table`, `BI_call_indirect`, `BI_memory_init`, `BI_data_drop`, `ME_functions`); spelling-only hardening against future scope-inference breakage, with `tests/test_data/rocq/rocq_prime_example.v` regenerated ([#344])
- Negative integer constants now reach the emitted `.v` parenthesized — `BI_const_num (Vi32 (-1))` — fixing `coqc` rejection of any module containing a negative constant, including unsigned literals stored as negative ([#314])
- The WASM module-name subsection now reflects the input file stem instead of the hardcoded `"output"`, so emitted `Definition`/`Theorem` identifiers use the source filename and multi-module workflows no longer collide
- Empty per-spec lists emit `(@nil hassert)` instead of `[]%N`, so the generated `Definition` type-checks regardless of active scope; proof scripts matching `[]%N` or `(@nil N)` literally must update ([issue#21], [issue#22])
