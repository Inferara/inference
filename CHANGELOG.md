...
- Element segments, data segments, and `br_table` now emit terms the proof contract accepts. Two of the old spellings were outright `coqc` errors, so any module carrying a data segment or a function-index element segment produced an unusable `.v`: a data byte was written `#78`, a notation no library defines, and an element segment's function indexes were written as an `ME_functions` element *mode* into `modelem_init`, the field that holds initializer expressions. The other two type-checked only because the vendored stub had drifted along with the emitter — the false green its own README warns about — declaring `ME_declared` where vanilla WasmCert-Coq has `ME_declarative`, and a one-argument `BI_br_table` that silently dropped the default label, making the emitted instruction a different instruction from the module's; that one was a hard error only for a table with no explicit targets, which emitted a bare, partially-applied `BI_br_table`. Now `BI_br_table` carries the label vector *and* the default (`BI_br_table (0%N :: 1%N :: nil) 1%N`, and `BI_br_table nil 0%N` for `br_table 0`), function-index element items desugar to the `BI_ref_func` initializer expressions `modelem_init` is typed for, `ME_declarative` and the standard-library `byte` constructor for a byte's value (`x78`) take the drifted spellings' place, and a `ref.func` operand is renumbered past omitted spec functions like every other function index, so both element item forms reach the same index instead of only the shorthand one. All of these are unreachable from Inference codegen and arrive only from foreign or statically-linked `.wasm`, so no committed `.v` golden or corpus output moves; a handcrafted foreign module added to the `coqc` gate covers them ([#346])
...
- `infs` unit tests that write an executable stub and then spawn it no longer fail intermittently with `ETXTBSY` ("text file busy"). Rust opens files `O_CLOEXEC`, but `CLOEXEC` closes a descriptor only *at* `exec`, so a child forked by another thread between the stub writer's `open` and its `close` carries a copy of the write descriptor until it reaches its own `exec` — and an `execve` of the stub inside that window is refused. The unit-test binary spawns processes throughout, which is how `optimize_in_place_leaves_original_on_wasm_opt_failure` came to report `Failed to execute wasm-opt at /tmp/.tmp5eIQbx/wasm-opt` on an ubuntu CI run. A shared `retry_while_exec_busy` helper now retries such a spawn while the error chain carries an `ExecutableFileBusy` I/O error (50 attempts, 20 ms apart), passing `Ok` and every other error straight through so a test asserting a genuine failure is unaffected; where the code under test *swallows* the spawn error instead of surfacing it — `probe_wasm_opt_version` maps a failed `--version` to `None` — the stub is settled at the point it is written, by running it until the `exec` succeeds. The audit of every sibling stub found the remaining ones out of reach: they are only resolved by `which` (mode bits, no `exec`), executed by a child `infs` process, or written by a `rustc` child rather than the test process; `probe_flag` in `project_build.rs` already carried its own retry. The failing assertion in `optimize_in_place_leaves_original_on_wasm_opt_failure` now interpolates `{err:#}`, so the errno-bearing source lands in the CI log rather than only the outermost context ([#345])
- The tests outside `apps/infs` that named a fixed temporary path no longer race across concurrent test processes — the class [#331] closed inside `apps/infs` and left standing everywhere else. Every scratch path now comes from a `tempfile::TempDir`, named through the operating system's exclusive-create loop and removed on drop. A `fastrand` suffix is deliberately not used: its thread-local seed hashes a coarse clock through fixed-key SipHash, so two processes landing on the same tick draw the same name (measured under #331, below). Three sites, two of them live defects ([#343]):
  - `inference-wasm-codegen`'s `write_wasm_to_creates_file` wrote, read back, and removed the fully constant `<temp>/test_codegen_output_wasm.wasm`, so an overlapping process could delete the file between another's write and its read. The directory's drop guard also replaces the manual cleanup, which only ran on the success path
  - `inference-tests`' `algo_array_cross_compiler_test` and `algo_iter_cross_compiler_test` built their `rustc` and `zig` reference modules into the constant `/tmp/algo_{array,iter}_{rustc,zig}` directories and read them back, so on any machine carrying both external compilers two overlapping runs raced on the same files — and in the `algo_iter` case shared `zig`'s build directory as well. They escaped the first sweep by spelling the path as a `/tmp` literal instead of `env::temp_dir()`; that literal also sent the outputs to a drive-relative `\tmp\…` on Windows rather than the platform temporary directory, and left every run's artifacts behind
  - `inference-project-model`'s `scan_of_unreadable_root_is_empty_not_a_give_up` used a constant directory name. Benign as written — the path is only asserted absent, never created — but a shape that invites a non-benign copy; it now scans a never-created child of the file's own per-run `TempProject` root
- `infs` unit tests no longer collide on their temporary directories, so two overlapping test processes (a filtered run beside a full one, or CI matrix jobs sharing a runner workspace) stop tearing down each other's fixtures. Every scratch directory now comes from an `assert_fs::TempDir`, which names it through the operating system's exclusive-create loop and removes it on drop — so the manual cleanup that only ran on the success path is gone too. Two independent defects were behind it, both reproduced by running copies of the unit-test binary concurrently ([#331]):
  - Tests in `toolchain/{paths,doctor,verify,shell,conflict,resolver}.rs` used a *constant* path (`env::temp_dir().join("infs_test_…")`) and removed and recreated it, so a concurrent process mid-assertion saw its own files vanish. Two processes filtered to `toolchain::` failed on every attempt, ten of ten, with ten distinct tests down per run: `AlreadyExists` from `create_dir_all`, `NotFound` on a fixture write, a symlink whose source had disappeared, and the originally reported `read_infs_metadata_no_warning_when_version_matches` failing its "metadata should be returned for a fresh install" expectation
  - The `project/scaffold.rs`, `toolchain/archive.rs`, and `toolchain/binaryen.rs` helpers looked safe — they appended `fastrand::u64(..)` to the directory name — but that suffix is not unique across processes. `fastrand`'s thread-local seed is a `DefaultHasher` (fixed SipHash keys, unlike `RandomState`) over `Instant::now()` and `ThreadId`; the thread id is a per-process sequential counter, so the *only* entropy is the clock, and on Apple Silicon `Instant` advances in ~41.67 ns steps. Two processes reaching the same test on the same tick draw the same u64 and land in the same "unique" directory — measured directly: 10 duplicate directory names across 180 concurrent processes, and 4 failed runs in 300 with symptoms that look nothing like a temp-dir bug (a truncated gzip stream read as "unexpected end of file", and an extracted archive silently missing a file because the other process rewrote it mid-read)
- `infs` unit tests no longer write the process-global `INFS_VERBOSE`, so nothing races the reads that production code makes of it. `serial_test` serializes only *marked* tests, and the six marked writers overlapped three unmarked tests that reach a read through production code: `broken_exe_override_falls_through_to_none` and `canonicalize_fails_but_raw_matches_shape_without_sibling` (whose fabricated paths always fail `canonicalize`, so `workspace_sibling_infc` always evaluates its fallback trace), and `post_build_optimize_skips_proof_and_v_builds` (whose enabled `[build.wasm-opt]` reaches the verification-build skip). An unsynchronized read/write pair is undefined behavior, not merely a flaky assertion, and it was observable: with the writers and readers filtered into one 16-thread run, 300 runs printed 15 verbose-only trace lines from tests that never set the variable — 0 in 300 after the fix. Marking the readers too would have left the same trap for the next test that reaches a read, so the writes are gone instead: the value predicate moved into a pure `verbose_from(Option<&OsStr>)` that the `resolver` tests call directly, and `read_infs_metadata` grew a `read_infs_metadata_with_verbosity` core that takes the flag as a parameter (mirroring `resolve_wasm_opt_from`, where the environment read is already lifted for the same reason). No test in the crate needs serializing for that variable now ([#331])
...
- wasm-to-v: the `(*name*)` comments on `BI_local_get` / `BI_local_set` / `BI_local_tee` are now attributed to the function they belong to. The local-name map decoded from the WASM `name` section is keyed by **function** index, but the translator resolved it with the function's **type** index, so wherever the two diverge a body was labelled with a stranger's parameter and local names — or lost its comments entirely, when no function held that index as a name-map key. A single-file `infc` build was never affected: the code generator pushes a fresh type entry per defined function and interns signatures only for imports, so a defined function's type index there always equals its function index, and every committed `.v` golden is byte-identical. Linking is where the two part company, because the static merge interns signatures across the merged modules: in an `infc -L -v` build of `external fn inc(a: i32) -> i32; external fn dec(a: i32) -> i32;` against an `arith` module exporting both, all three functions of the linked artifact share type entry 0, and the two folded-in external bodies were annotated with the entry function's parameter name. WASM handed to the public `wasm_to_v` entry point by another producer diverges the same way — a hand-written or `wat`-assembled module also splits the two numberings through a function import, since the name section counts imports and the function section does not, leaving the first defined body without its name comments and every later body wearing an earlier function's names. The lookup now uses the same absolute function index the rest of the name-section handling already uses. Comments only: the translated instruction stream is untouched, so no proof obligation changes ([#336])
