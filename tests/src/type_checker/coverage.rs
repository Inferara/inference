...
mod statement_coverage {
    ...
    fn test_loop_with_non_bool_condition() {
        ...
        let result = try_type_check(source);
        always!(result.is_err(), "Loop with non-bool condition should fail");
    ...
    fn test_if_with_non_bool_condition() {
        ...
        let result = try_type_check(source);
        always!(result.is_err(), "If with non-bool condition should fail");
        if let Err(error) = result {
            let error_msg = error.to_string();
            always!(
                error_msg.contains("type mismatch") || error_msg.contains("expected Bool"),
    ...
    fn test_assert_statement_with_non_bool() {
        ...
        let result = try_type_check(source);
        always!(result.is_err(), "Assert with non-bool should fail");
        if let Err(error) = result {
            let error_msg = error.to_string();
            always!(
                error_msg.contains("type mismatch") || error_msg.contains("expected Bool"),
    ...
    fn test_assign_statement_type_mismatch() {
        ...
        let result = try_type_check(source);
        always!(result.is_err(), "Assignment with type mismatch should fail");
        if let Err(error) = result {
            let error_msg = error.to_string();
            always!(
                error_msg.contains("type mismatch"),
...
mod expression_coverage {
    ...
    fn test_array_literal_type_mismatch() {
        ...
        let result = try_type_check(source);
        always!(result.is_err(), "Array with mismatched types should fail");
        if let Err(error) = result {
            let error_msg = error.to_string();
            always!(
    ...
    fn test_array_index_on_non_array() {
        ...
        let result = try_type_check(source);
        always!(result.is_err(), "Array indexing on non-array should fail");
        if let Err(error) = result {
            let error_msg = error.to_string();
            always!(
    ...
    fn test_binary_logical_and_non_bool() {
        let source = r#"fn test() -> bool { return 1 && 2; }"#;
        let result = try_type_check(source);
        always!(result.is_err(), "Logical AND with non-bool should fail");
        if let Err(error) = result {
            let error_msg = error.to_string();
            always!(
    ...
    fn test_binary_logical_or_non_bool() {
        let source = r#"fn test() -> bool { return 1 || 2; }"#;
        let result = try_type_check(source);
        always!(result.is_err(), "Logical OR with non-bool should fail");
        if let Err(error) = result {
            let error_msg = error.to_string();
            always!(
                error_msg.contains("invalid") || error_msg.contains("logical"),
    ...
    fn test_binary_arithmetic_with_non_number() {
        ...
        let result = try_type_check(source);
        always!(result.is_err(), "Arithmetic on non-numbers should fail");
        if let Err(error) = result {
            let error_msg = error.to_string();
            always!(
    ...
    fn test_unary_not_on_non_bool() {
        let source = r#"fn test() -> i32 { return !42; }"#;
        let result = try_type_check(source);
        always!(result.is_err(), "Unary NOT on non-bool should fail");
        if let Err(error) = result {
            let error_msg = error.to_string();
            always!(
    ...
    fn test_unary_neg_on_unsigned_integer() {
        ...
        let result = try_type_check(source);
        always!(result.is_err(), "Unary neg on unsigned integer should fail");
        if let Err(error) = result {
            let error_msg = error.to_string();
            always!(
    ...
    fn test_unary_neg_on_bool() {
        let source = r#"fn test() -> bool { return -true; }"#;
        let result = try_type_check(source);
        always!(result.is_err(), "Unary neg on bool should fail");
        if let Err(error) = result {
            let error_msg = error.to_string();
            always!(
                error_msg.contains("unary operator") || error_msg.contains("signed"),
    ...
    fn test_unary_bitnot_on_bool() {
        let source = r#"fn test() -> bool { return ~true; }"#;
        let result = try_type_check(source);
        always!(result.is_err(), "Unary bitnot on bool should fail");
        if let Err(error) = result {
            let error_msg = error.to_string();
            always!(
    ...
    fn test_struct_expression_undefined() {
        ...
        let result = try_type_check(source);
        always!(result.is_err(), "Undefined struct expression should fail");
        if let Err(error) = result {
            let error_msg = error.to_string();
            always!(
    ...
    fn test_member_access_on_non_struct() {
        ...
        let result = try_type_check(source);
        always!(result.is_err(), "Member access on non-struct should fail");
        if let Err(error) = result {
            let error_msg = error.to_string();
            always!(
...
mod function_registration_coverage {
    ...
    fn test_self_reference_in_function() {
        ...
        let result = try_type_check(source);
        always!(result.is_err(), "Self reference in function should fail");
        if let Err(error) = result {
            let error_msg = error.to_string();
            always!(
...
mod generic_type_inference_coverage {
    ...
    fn test_conflicting_type_inference() {
        ...
        let result = try_type_check(source);
        always!(result.is_err(), "Conflicting type inference should fail");
        if let Err(error) = result {
            let error_msg = error.to_string();
            always!(
...
mod import_resolution_coverage {
    ...
    fn test_partial_import_nonexistent_path() {
        ...
        let result = try_type_check(source);
        always!(result.is_err(), "Import from nonexistent path should fail");
    }
}
