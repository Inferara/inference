...
pub(crate) mod gate {
    ...
    /// side.
    ///
    /// The last three entries share `spec_linked_extern.inf`'s external and
    /// differ only in the channel the obligation reaches the merged body
    /// through, which is the axis nothing else in the corpus varies.
    /// `spec_linked_exists.inf` and `spec_linked_unique.inf` put the merged
    /// ordinal inside a `reach_payload`, so the module `coqc` elaborates is a
    /// merged one carrying a reachability obligation — the combination the
    /// `ValidExistsSpec`/`ValidUniqueSpec` runtime constraint applies to, since
    /// those predicates need an import-free module and the merge is what empties
    /// the import list. The two are not interchangeable: the kinds are gathered
    /// into different partitions and consumed by different predicates.
    /// `spec_linked_app_ok.inf` states `HA_app_ok` over a merged body, which is
    /// a claim of a different order from applying one — an application says what
    /// the body computes, realization says that it computes at all, so a body
    /// that trapped would falsify the second and not the first.
    ///
    ...
    pub(crate) const LINKED_CORPUS: &[(&str, &str, &[LinkedExternal], LinkOptions)] = &[
        (
    ...
    fn shared_generator_preserves_existing_output() {
        ...
        let shared = generate_v("rocq_exists_spec.inf", "rocq_exists_spec");
    ...
    /// The `.v` for one [`LINKED_CORPUS`] entry, looked up by fixture name.
    ///
    /// The lookup is what holds a per-entry gate to the entry still being in the
    /// list: a test that generated the fixture itself would keep passing after
    /// the entry was deleted, having stopped measuring anything the gate
    /// compiles.
    fn linked_corpus_entry_v(fixture: &str) -> String {
        let (source, module, externals, options) = LINKED_CORPUS
            .iter()
            .find(|(source, _, _, _)| *source == fixture)
            .unwrap_or_else(|| {
                panic!("{fixture} must stay a LINKED_CORPUS entry for this gate to measure it")
            });
        generate_linked_v(source, module, externals, options)
    }

    /// The one `reach_payload` in a single-obligation reachability module.
    ///
    /// The payload is emitted on one line, and requiring exactly one is what
    /// makes a needle over it an assertion about *this* fixture's obligation
    /// rather than about whichever obligation happened to carry the shape.
    fn sole_reach_payload<'v>(v: &'v str, fixture: &str) -> &'v str {
        let payloads: Vec<&str> = v
            .lines()
            .filter(|line| line.contains("reach_payload := "))
            .collect();
        assert_eq!(
            payloads.len(),
            1,
            "{fixture} must state exactly one reachability obligation; found {}:\n{v}",
            payloads.len()
        );
        payloads[0]
    }

    /// The `mod_funcs` ordinal of the body merged in from `mathlib`.
    ///
    /// Read off the record rather than written down, so a reordering fails as a
    /// missing definition instead of quietly moving what an index means.
    fn merged_mathlib_index(v: &str, fixture: &str) -> usize {
        let defined = module_func_names(v);
        defined
            .iter()
            .position(|name| *name == "mathlib_double")
            .unwrap_or_else(|| {
                panic!("{fixture} must merge `mathlib::double`; it defines {defined:?}\n{v}")
            })
    }

    /// The `exists` entry's own floor: its reachability payload applies a body
    /// the compiler never emitted.
    ///
    /// [`linked_corpus_carries_a_merged_body`] holds every linked entry to
    /// carrying *a* merged body some obligation applies, which this entry would
    /// satisfy through its executable `twice` alone. What only this entry can
    /// claim is that the merged ordinal reaches a *reachability* payload —
    /// `rocq_exists_spec.inf` produces the same `ValidExistsSpec` grammar with
    /// every application naming a function from its own translation unit, so
    /// neither `ValidExistsSpec` nor a bare `T_app` distinguishes the two.
    ///
    /// The pairing is the content: a reachability predicate quantifies a module
    /// runtime, whose construction needs an empty import list, and a module that
    /// calls an external has one only after the merge. A regression that stopped
    /// merging would leave an unsatisfiable import; one that stopped resolving
    /// the applied symbol onto the merged body would leave the payload naming
    /// something else.
    #[test]
    fn the_exists_entry_applies_a_merged_body_in_its_reachability_payload() {
        const FIXTURE: &str = "spec_linked_exists.inf";

        let v = linked_corpus_entry_v(FIXTURE);
        let index = merged_mathlib_index(&v, FIXTURE);
        let payload = sole_reach_payload(&v, FIXTURE);

        assert!(
            payload.contains(&format!("T_app {index} ")),
            "{FIXTURE} merges `mathlib::double` at record index {index}, and its \
             reachability payload applies something else — the entry claims linked \
             coverage of the `exists` kind that it does not have. That payload \
             was:\n{payload}"
        );
        assert!(
            v.contains(
                "Theorem valid_exists_spec_linked_exists__ReachableExtern : \
                 ValidExistsSpec spec_linked_exists \
                 spec_linked_exists__ReachableExtern__ex_specs."
            ),
            "the payload must be consumed by a `ValidExistsSpec` theorem; the two \
             reachability kinds select different predicates, so a payload landing \
             in the other partition is not this entry's coverage:\n{v}"
        );
    }

    /// The `unique` entry's own floor, and not a copy of the `exists` one: the
    /// two kinds are gathered into different partitions and consumed by
    /// different predicates, so a merged ordinal reaching an `__ex_specs`
    /// payload says nothing about one reaching a `__uq_specs` payload.
    #[test]
    fn the_unique_entry_applies_a_merged_body_in_its_reachability_payload() {
        const FIXTURE: &str = "spec_linked_unique.inf";
        ...
        assert!(
            ...
            "{FIXTURE} merges `mathlib::double` at record index {index}, and its \
             reachability payload applies something else — the entry claims linked \
             coverage of the `unique` kind that it does not have. That payload \
             was:\n{payload}"
        );
        assert!(
            v.contains(
                "Theorem valid_unique_spec_linked_unique__UniqueExtern : \
                 ValidUniqueSpec spec_linked_unique \
                 spec_linked_unique__UniqueExtern__uq_specs."
            ),
            "the payload must be consumed by a `ValidUniqueSpec` theorem; the two \
             reachability kinds select different predicates, so a payload landing \
             in the other partition is not this entry's coverage:\n{v}"
        );
    }

    /// The realization entry's own floor: `HA_app_ok` names a merged body.
    ///
    /// `spec_bounds_realization.inf` in [`CORPUS`] already emits `HA_app_ok`, so
    /// the bare atom is not coverage of anything linked, and the shared floor
    /// reads `T_app` rather than `HA_app_ok` — an entry whose obligation only
    /// *applied* the merged body would satisfy it while claiming nothing about
    /// that body terminating. The ordinal is the whole assertion: realization is
    /// a total-correctness judgment, and this is the only place the corpus makes
    /// one about foreign code. Both needles are read off the fixture's single
    /// obligation term rather than the whole file, which is what makes the
    /// second one a claim that realization and application travel together: a
    /// module stating them in two separate obligations satisfies a whole-file
    /// needle for each while claiming neither about the other.
    #[test]
    fn the_realization_entry_claims_a_merged_body_is_realized() {
        const FIXTURE: &str = "spec_linked_app_ok.inf";

        let v = linked_corpus_entry_v(FIXTURE);
        let index = merged_mathlib_index(&v, FIXTURE);
        let terms = obligation_terms(&v);
        assert_eq!(
            terms.len(),
            1,
            "{FIXTURE} must state exactly one obligation for the needles below to \
             be about it rather than about whichever obligation carried the \
             shape; found {}:\n{v}",
            terms.len()
        );
        let term = terms[0];
        ...
        assert!(
            term.contains(&format!("HA_app_ok {index} ")),
            "{FIXTURE} merges `mathlib::double` at record index {index} and no \
             obligation claims that application is realized, so the merged body is \
             elaborated with nothing said about it terminating:\n{term}"
        );
        assert!(
            term.contains(&format!("T_app {index} ")),
            "the realization atom and the application travel together in this \
             fixture; without the second the obligation would claim the call \
             completes while claiming nothing about what it computes. Both are \
             read off the one obligation term, so a fixture that split them \
             across two obligations fails here:\n{term}"
        );
    }
...
"#;
    ...
    /// Committed `.v` golden for the linked-external fixture, a selected
    /// discharge artifact. Two comparisons hold it to its producer:
    /// [`producer_golden_linked_extern_matches_committed_v`] here, through the
    /// [`LINKED_CORPUS`] entry, and `rocq_dischargeability`'s export, which
    /// regenerates it from its own case list and refuses to publish on a
    /// mismatch. The generator is shared; the inputs are stated twice, and the
    /// discharge suite's cross-list guard pins them equal, so a divergence in
    /// either input is a red test that names the literal, not a silent split
    /// between the bytes a companion proof is written against and the bytes
    /// `coqc` elaborates. Regenerated with the `#[ignore]`d
    /// [`regenerate::regenerate_linked_extern_v`].
    fn linked_extern_golden_path() -> PathBuf {
        get_test_data_path().join("rocq").join("spec_linked_extern.v")
    }
    ...
    /// The linked-external golden's other comparison runs in the discharge
    /// suite under the build workflow, not under the gate this module feeds.
    /// Comparing here as well keeps the gate that elaborates the merged
    /// module's `.v` self-contained about the bytes a companion proof was
    /// written against: a producer change that moves them is red in the same
    /// job that elaborates them.
    #[test]
    fn producer_golden_linked_extern_matches_committed_v() {
        let generated = linked_corpus_entry_v("spec_linked_extern.inf");
        let golden_path = linked_extern_golden_path();
        let golden = std::fs::read_to_string(&golden_path).unwrap_or_else(|e| {
            panic!(
                "read {} ({e}); regenerate with `cargo test -p inference-tests \
                 regenerate_linked_extern_v -- --ignored`",
                golden_path.display()
            )
        });
        assert_eq!(
            generated,
            golden,
            "proof-mode `.v` for the linked spec_linked_extern.inf drifted from {}",
            golden_path.display()
        );
    }
    ...
    mod regenerate {
        ...
        #[test]
        #[ignore]
        fn regenerate_linked_extern_v() {
            let v = linked_corpus_entry_v("spec_linked_extern.inf");
            write_golden(&v, &linked_extern_golden_path());
        }
