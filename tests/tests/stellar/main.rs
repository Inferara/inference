...
//! validator runs.
//!
//! Three things live here. `envelope` pins what the host accepts and refuses of
//! a module's *shape* — the mandatory metadata section, the declared protocol,
//! floats, multi-value, an exported mutable global. `wrappers` measures the
//! `Val` marshalling instruction sequences themselves: each shape is assembled,
//! executed and round-tripped, and the sequence that actually works is recorded
//! in `MEASURED_ABI.md` beside this file. `contracts` leaves hand assembly
...
//! answer exactly what the default build answers under `wasmtime`.
//!
