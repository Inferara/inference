...
impl ModuleState {
    ...
    fn check_const_expr(
        ...
        macro_rules! define_visit_operator {
            ...
            (@visit $self:ident visit_uzumaki_i32 $val:ident) => {{
                $self.validator().visit_uzumaki_i32($val)
            }};
            (@visit $self:ident visit_i64_const $val:ident) => {{
                $self.validator().visit_i64_const($val)
            }};
            (@visit $self:ident visit_uzumaki_i64 $val:ident) => {{
                $self.validator().visit_uzumaki_i64($val)
            }};
