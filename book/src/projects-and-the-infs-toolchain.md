...
all four forward a `[host-imports]` table too. The unknown-key refusal first
shipped in `v0.0.6`, though: a release through `v0.0.5` reads past a root table
it does not know with no diagnostic, and for `[host-imports]` that means
building under no policy at all, so a project that relies on its allowlist has
to be built with an `infs` that knows the table. See
