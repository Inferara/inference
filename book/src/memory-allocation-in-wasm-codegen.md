...
2. **It reaches an `external fn` argument whose declaration marks any parameter `mut`.** A linked external shares the program's single linear memory and receives a compound argument as a raw pointer, so its body can store through that pointer. `mut` on one of the external's declared parameters is the statement that this may happen ([External Functions and WASM Linking](external-functions-and-wasm-linking.md)), and the linker checks the claim against the merged bytes at link time, rejecting a body that stores where its declaration did not permit. A parameter that flows to such a call keeps its copy — and it keeps it for **every** position the call reaches it at, not only the `mut` one: the gate decides per external, not per parameter, because narrowing further would rest on a containment property the linker's provenance proof does not establish (see [The WASM Linker](the-wasm-linker.md)). A parameter that reaches only externals declaring **no** `mut` parameter at all is passed by reference like any other unwritten parameter — the linker requires such a closure to record no store anywhere, so eliding the caller's copy is unobservable.
...
Note what decides each half of this, because the two halves are decided differently. Whether the *body* assigns through the parameter is still a fact about the body alone: the `mut` marker on an Inference parameter is a contract with the type checker — it states whether the function is permitted to assign through the parameter, and a program that assigns through a parameter declared without it is rejected — but it is not itself the input to this decision, so a `mut` parameter the body never assigns through is passed by reference exactly like a non-`mut` one. Deciding on the marker instead would mean that dropping a `mut` a function does not need makes the program faster, putting the annotation's cost in opposition to its purpose. Whether the parameter reaches a *writing* external, by contrast, is a fact about that external's own declaration: codegen reads the callee's `mut` markers, because the merged `.wasm` body itself is bytes the compiler never sees — there is no body to scan the way there is for an Inference-level assignment. Neither lowering is observable from Inference source, so there is no reason to write a program one way rather than another in order to obtain one of them.
...
A method receiver is a parameter, and the rule above is the whole rule for it too. A method that assigns through `self` copies the receiver on entry; a method that only reads it does not, whether the receiver is written `self` or `mut self`. Forwarding an immutable receiver to a `mut` `external fn` position is a different matter, covered next.

The external case is the one place where a write is invisible in Inference source: linking merges every module into one linear memory, a compound `external fn` parameter is lowered to a raw `i32` pointer with no copy between the call site and the foreign body, and the foreign body is free to store through that pointer. `mut` on an `external fn` parameter is the declaration that this may happen, and the linker checks the claim against the merged bytes at link time — a body that stores where its declaration did not permit fails to link. Because such a write can be invisible in source, the language does not let it happen to a binding that never said it could change: passing a struct or array argument — or any projection of one, such as `self.field` or `arr[i]` — to a `mut` external parameter requires the projection's own binding (the receiver or the parameter it is rooted at) to be `mut` itself, enforced at the call site by analysis rule A047. An immutable `self` or an immutable compound parameter can therefore no longer be forwarded to a `mut` external position at all. Codegen's copy rule follows the same declaration: a parameter that reaches an external declaring any `mut` parameter keeps its copy (see [When a Copy Is Emitted](#when-a-copy-is-emitted)); one that reaches only externals declaring no `mut` parameter at all does not, since the linker then requires the closure to record no store anywhere. The linker's side of this arrangement — the write set it derives from the merged bytes, and precisely what it does and does not prove about where a declared store lands — is described in [The WASM Linker](the-wasm-linker.md).
...
Zig also uses `--stack-first` layout when targeting WASM, placing the stack at low addresses so overflow naturally traps. Inference matches this layout. Zig zero-initializes local arrays (matching Inference's `memory.fill` approach). Zig's safety modes add bounds checks on array access under a build mode; Inference emits its bounds guard in every build, proof mode included, because the artifact a proof is written about has to be the artifact that ships.
...
### Bounds Checking as a Proof Obligation

Array index access is bounds-checked: a *literal* out-of-bounds index is rejected at compile time (analysis rule A037), and any other index emits a runtime guard (`index >= length -> unreachable`). The guard is emitted in **every** build, Proof mode included, so the artifact a proof is written about is byte-for-byte the artifact that ships.

The line between the two halves is narrower than "constant" suggests. Code generation folds a direct number literal and nothing else, so `arr[3]` needs no guard while `arr[K]` for a `const K` — and `arr[1 + 1]` — take the guarded path. A037 draws its line in the same place, wanting the literal directly under the access, so a named or out-of-range constant in an executable function has no static story at all and the runtime guard is what catches it.

That guard is what makes the bound provable. A proof-mode dynamic access used to lower to a bare `BI_load`/`BI_store` carrying no side condition, leaving the emitted `.v` with nothing to be about; it now lowers to the guard, and the guard is a trap the Rocq contract can already see.

The obligation that forces the bound is one the contract already had, not a new predicate. `HA_app_ok f τs` — the assertion a bare call statement in a specification body emits — denotes that the interpretation is defined at `f(τs)`, and realizing that claim requires proving `fun_computes`, which is stated through a **total-correctness** judgment: the body must reduce to a value stack, never to a trap. So claiming a call is realized is claiming its body does not trap, and a body containing the guard cannot be trap-free unless the index is in range at every access. A specification that declares its envelope with `assume` and then calls the function therefore carries `0 <= i < array_length` as a real, dischargeable obligation.

Extending `ValidModule` with a blanket "no reachable trap" clause would not have worked, and it is worth saying why. `ValidModule` is structural well-formedness — WasmCert's `module_typing` — and reduces nothing. More decisively, `unreachable` is emitted from six sites in seven roles, and in two of them trapping is the *mechanism* rather than a defect: an ordinary executable `assert`, whose entire purpose is to trap when its condition is false, and every reachability filter in a retained specification body, which the judgment counts on to discard the choice vectors the specification rejects. A blanket clause would make `assert` unusable in Proof mode and falsify every reachability obligation at once. The bounds guard, the narrow signed-division overflow guard, an exported entry's `enum` tag guard and the overflow guard on a `+`, `-`, `*` or unary `-` are the opposite case — a conditional trap whose condition is a property worth proving — and a value-returning function's dead tail is neither, since analysis rule A007 is what keeps it unreached. Trap-freedom belongs where a programmer asks for it, per call, which is what `HA_app_ok` gives.

One shape is refused rather than obligated: a non-constant index inside an `exists`/`unique` body (diagnostic P016). Those bodies are the ones the reachability judgment reduces, and it fixes an arbitrary typed entry vector before the choices range — so an entry whose index trips the guard leaves an empty observation set and makes the theorem *false* rather than narrower. P016 folds the wider notion of constant, so `a[K]` passes it and is then guarded anyway by the narrower one; that guard is dead, because the same pass has already checked `K` against the array's declared length — the length the guard compares against.

Two elisions remain future work, and they are not the same proposal. Issue #215 is a range/dominator analysis that proves a *particular* access in bounds and drops that one guard; it trades nothing away, because the guard it removes was provably dead, and every access it cannot prove keeps its guard. Dropping a *live* guard in the deployed build on the strength of a module having been proven is #214's own step 3, left unimplemented: it would trade away the property described above, since the artifact that ships would no longer be the artifact that was proven, and no mechanism for it exists today — there is no proof-status type, attestation, or manifest key to stand behind it.

The full statement lives in `core/wasm-to-v/ROCQ_CONTRACT.md`.
...
| `wasm_codegen_param_escapes_to_extern` | The parameter reaches an argument of an `external fn` declaring a `mut` parameter, so it keeps its copy |
| `wasm_codegen_param_reaches_read_only_extern` | The parameter reaches an `external fn` argument, but every declaration it reaches marks nothing `mut`, so it is still passed by reference |
