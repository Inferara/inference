...
Extending `ValidModule` with a blanket "no reachable trap" clause would not have worked, and it is worth saying why. `ValidModule` is structural well-formedness — WasmCert's `module_typing` — and reduces nothing. More decisively, `unreachable` is emitted from five sites in six roles, and in two of them trapping is the *mechanism* rather than a defect: an ordinary executable `assert`, whose entire purpose is to trap when its condition is false, and every reachability filter in a retained specification body, which the judgment counts on to discard the choice vectors the specification rejects. A blanket clause would make `assert` unusable in Proof mode and falsify every reachability obligation at once. The bounds guard, the narrow signed-division overflow guard and an exported entry's `enum` tag guard are the opposite case — a conditional trap whose condition is a property worth proving — and a value-returning function's dead tail is neither, since analysis rule A007 is what keeps it unreached. Trap-freedom belongs where a programmer asks for it, per call, which is what `HA_app_ok` gives.
